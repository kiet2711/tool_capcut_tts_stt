...
class VODUploader:
    ...
    def upload_file(self, file_path: Union[str, Path]) -> UploadResult:
        ...
        # 3. Transfer binary in chunks
        chunk_size = 5 * 1024 * 1024  # 5MB chunks
        chunks = [data[i:i + chunk_size] for i in range(0, len(data), chunk_size)]
        if not chunks:
            chunks = [b""] # handle empty file edge case
            
        part_crcs = []
        for i, chunk in enumerate(chunks):
            chunk_crc32 = crc32_hex(chunk)
            part_crcs.append(f"{i}:{chunk_crc32}")
            
            transfer_url = f"https://{upload_host}/upload/v1/{store_uri}?" + urlencode(
                {"uploadid": upload_id, "part_number": str(i), "phase": "transfer"}
            )
            transfer_resp = self.session.post(
                transfer_url, headers=self._upload_binary_headers(upload_auth, chunk_crc32), data=chunk, timeout=300
            )
            _checked_json_response(transfer_resp, f"upload transfer part {i}")
