...
class DeviceConfig:
    ...
    def randomize(self):
        """Randomize the device ID to simulate a fresh installation and bypass rate limits."""
        import random
...
class SubtitleResult:
    ...
    def from_payload(cls, payload: Dict[str, Any]) -> "SubtitleResult":
        ...
        for item in raw_utterances:
            ...
            # Lấy bản dịch nếu có, nếu không thì lấy văn bản gốc
            text = item.get("translation_text") or item.get("text", "")
            if text:
